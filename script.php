...
<script type="application/javascript">
    ...
    types[<?php echo $carId;?>] = [ <?php
        for ($i = 0; $i < count($types) - 1; $i++ ) {
            ?>{ id: <?php echo $types[$i]['id']; ?>, name: "<?php echo $types[$i]['type']; ?>" }, <?php
        }
        ?>{ id: <?php echo $types[$i]['id']; ?>, name: "<?php echo $types[$i]['type']; ?>" } ];
    <?php
    }
    ?>
...
const years = Array();
    ...
    years[<?php echo $carId;?>] = [ <?php
        for ($i = 0; $i < count($years) - 1; $i++ ) {
            ?>{ id: <?php echo $years[$i]['id']; ?>, name: "<?php echo $years[$i]['year']; ?>" }, <?php
        }
        ?>{ id: <?php echo $years[$i]['id']; ?>, name: "<?php echo $years[$i]['year']; ?>" } ];
    <?php
    }
    ?>
